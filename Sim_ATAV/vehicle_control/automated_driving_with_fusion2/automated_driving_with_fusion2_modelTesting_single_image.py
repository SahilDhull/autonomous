...
class NetworkLight(nn.Module):
    def __init__(self):
        ...
        self.linear_layers = nn.Sequential(
            nn.Linear(in_features=25920 + 3, out_features=30),
            nn.ELU(),
            # nn.Dropout(p=0.3),
    ...
    def forward(self, input, vel, direction):
        input = input.view(input.size(0), 3, 250, 600)
...
if model_val == 1:
    model1 = NetworkLight()
    state1 = torch.load(file_path + 'models/earlier_running_models/model1.h5')
    model1 = state1['model']
...
############ Model 2 ###############
    ...
    left_state = torch.load(file_path + 'models/new/model2/m2_left_20.h5')
    ...
    st_state = torch.load(file_path + 'models/Adam/new_model2/m2_left_6.h5')
...
def read(name):
    ...
    current_image = current_image[125:-25, :, :]
            ...
            if self.is_direct_speed_control:
                ...
                if cur_time_ms<3010:
                    ...
                    if cur_time_ms%100==0:
                        ...
                        # Model 1
                        if model_val == 1:
                            str_angle = MLmodel([img_name,cur_speed_ms, one_hot], model1)


                        # Model 2
                        elif model_val == 2:
                            if(direction == -1):
                                str_angle = MLmodel([img_name,cur_speed_ms, one_hot], left_model)
                            elif(direction == 1):
                                str_angle = MLmodel([img_name,cur_speed_ms, one_hot], right_model)
                            else:
                                str_angle = MLmodel([img_name,cur_speed_ms, one_hot], st_model)
