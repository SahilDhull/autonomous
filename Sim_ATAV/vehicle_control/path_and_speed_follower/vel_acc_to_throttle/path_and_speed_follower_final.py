...
class PathAndSpeedFollower(BaseCarController):
    ...
    def run(self):
        ...
        def compute_and_apply_control():
            ...
            # ------------ All Changes for vel acc map here ---------------------------------------------
            if self.is_direct_speed_control:
                global t1, v1, vel_acc_dict,start_time, flag, prev_a
                th = 1.0
                

                if cur_time_ms<start_time:
                    x = controller_commons.speed_ms_to_kmh(v1)
                    ...
                    self.set_throttle(th)
                    self.setSteeringAngle(control_steering)
                

                if cur_time_ms>=start_time-1000:
                    if flag == 0 and abs(round(cur_speed_ms,1)-cur_speed_ms)<0.05:
                        t1 = cur_time_ms
                        v1 = cur_speed_ms
                        # print("--> "+str(v1)+" time: "+str(t1))
                        flag = 1
                    if (flag ==1 and cur_time_ms-t1>30) or cur_time_ms-t1 in (50,100,200):
                        a = ((cur_speed_ms-v1)/(cur_time_ms-t1))*1000
                        if(a>=0.0 and abs(prev_a-a)>=0.02):         # limited the acceleration sampling rate
                        # if(a>=0.0 and abs(prev_a-a)>=0.05):       # if the sampling is very high, uncomment this
                            vel_acc_dict[(round(cur_speed_ms,2),round(a,2))] = th
                            prev_a = a
                        flag = 0
