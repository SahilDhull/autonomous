...
class Pedestrian (Supervisor):
    ...
    def run(self):
        ...
        opt_parser.add_option("--loop", type=int, default=0, help="Will Pedestrian loop over her trajectory?")
        ...
        while not self.step(self.time_step) == -1:
            if not self.is_stopped:
                ...
                if i < self.number_of_waypoints-1 or self.is_loop:
                ...
                for i in range(0, self.BODY_PARTS_NUMBER):
                    current_angle = self.angles[i][current_sequence] * (1 - ratio) + self.angles[i][
                        (current_sequence + 1) % self.WALK_SEQUENCES_NUMBER] * ratio
                    self.joints_position_field[i].setSFFloat(current_angle)

                # adjust height
                self.current_height_offset = self.height_offsets[current_sequence] * (1 - ratio) + self.height_offsets[
                    (current_sequence + 1) % self.WALK_SEQUENCES_NUMBER] * ratio
