...
def computeTargetPath():
    ...
    while(p[i][j]!=(-1,-1)):
        travel_path = [[float(grid_points[i][j][0]),float(grid_points[i][j][1])]] + stravel_path
        (i,j) = p[i][j]
...
def run_test(ego_init_speed_m_s=10.0, ego_x_pos=20.0, pedestrian_speed=3.0, sim_duration=50000, for_matlab=False):
    ...
    vhc_obj.current_position = [319.0, 0.35, 1.88]
    ...
    vhc_obj.controller_arguments.append('1')#vhc_id
    vhc_obj.controller_arguments.append('False')
    vhc_obj.controller_arguments.append('False')
    ...
    vhc_obj.sensor_array[-1].add_sensor_field('name', '"camerat"')
    #----------------new changes for camera--------------------
    vhc_obj.sensor_array.append(WebotsSensor())
    vhc_obj.sensor_array[-1].sensor_type = 'Camera' # 'Radar' #'DelphiESR'
    vhc_obj.sensor_array[-1].sensor_location = WebotsSensor.LEFT
    vhc_obj.sensor_array[-1].add_sensor_field('name', '"cameral"')

    vhc_obj.sensor_array.append(WebotsSensor())
    vhc_obj.sensor_array[-1].sensor_type = 'Camera' # 'Radar' #'DelphiESR'
    vhc_obj.sensor_array[-1].sensor_location = WebotsSensor.RIGHT
    vhc_obj.sensor_array[-1].add_sensor_field('name', '"camerar"')
    ...
    # target_pos_list = [[333.0, 3.2], [330.0, 3.2], [323.0, 4.16], [321.0, 4.61], [317.0, 7.26], [313.0, 9.51], [310.0, 12.4], [307.0, 16.2], [305.0, 19.4], [303.0, 25.1], [303.0, 100.0]] #left turn
    # target_pos_list = [[450.0, -2.26], [240.0, -0.5], [100.0, -2.26], [-115.0, -2.26]] #straight
    # target_pos_list = [[-50.0, 2.26], [50.0,-1.0], [100.0, 2.26], [500.0, 2.26]]
    # target_pos_list = [[260.0, 2.26], [291.0, 1.26], [298.0, 9.0], [298.0,500.0]] #right turn
    # target_pos_list = [[330.0, -2.2], [307.0, -2.2], [302.0, -9.0], [302.0, -500.0]]
    # target_pos_list = [[-50.0, 2.26], [50.0,0.5], [150.0, 2.26], [500.0, 2.26]]
    
    # target_pos_list = [[450.0, -2.71], [311.3, -2.71], [297.3, 8.3] , [297.3, 100]] #left
    ...
    # target_pos_list = [[500.0, -2.26], [-500.0, -2.26]]
    # target_pos_list = [[500.0, -66.26], [-500.0, -66.26]]
    # target_pos_list = [[-500.0, 2.26], [500.0, 2.26]]

    # city horizontal
    target_pos_list = [[106.26, 100.0], [106.26, -200.0]]
    ...
    sim_config.world_file = '../Webots_Projects/worlds/crossing_test.wbt'
