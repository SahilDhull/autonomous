"""Case Study #1 for IV Journal Work
----------------------------------------------------------------------------------------------------------
...
def divide_by_100(value):
    """Converts lateral position and color value from Covering array to their correct values as float."""
    return float(value / 100.0)


PARAMETER_CONVERSION_DICT = {'ego_lateral_position': divide_by_100,
                             'agent_1_color_r': divide_by_100,
                             'agent_1_color_g': divide_by_100,
                             'agent_1_color_b': divide_by_100,
                             'pedestrian_1_shirt_color_r': divide_by_100,
                             'pedestrian_1_shirt_color_g': divide_by_100,
                             'pedestrian_1_shirt_color_b': divide_by_100,
                             'pedestrian_1_pants_color_r': divide_by_100,
                             'pedestrian_1_pants_color_g': divide_by_100,
                             'pedestrian_1_pants_color_b': divide_by_100}
...
def get_initial_sample(exp_ind):
    ...
    ca_exp_file_name = environment_config_dict['exp_config_folder'] + environment_config_dict['exp_short_name'] + '_ca_2_3_way.csv'
    exp_ind = int(round(exp_ind))
    data_frame = covering_array_utilities.load_experiment_data(ca_exp_file_name)
    init_sample = []
    for field_name in FALSIFICATION_PARAMETER_INDEX_MAP:
        field_val = covering_array_utilities.get_experiment_field_value(data_frame, exp_ind, field_name)
        if field_name in PARAMETER_CONVERSION_DICT:
            field_val = PARAMETER_CONVERSION_DICT[field_name](field_val)
...
def define_sim_environment(parameters_dict):
    ...
    ego_x_pos = float(lane_number_to_lat_pos(2, 'N')) + parameters_dict['ego_lateral_position']
    ...
    cur_vhc_id += 1
    vhc_obj = sim_config_tools.create_vehicle_object()
    vhc_model = agent_1_model
    y_pos = -50.0
    x_pos = lane_number_to_lat_pos(1, 'N')
    ...
    cur_vhc_id += 1
    vhc_obj = sim_config_tools.create_vehicle_object()
    vhc_model = 'CitroenCZeroSimple'
    y_pos = -60.0
    vhc_obj.current_position = [lane_number_to_lat_pos(1, 'N'), 0.35, y_pos]
    vhc_obj.current_orientation = 0.0
    vhc_obj.rotation = [0.0, 1.0, 0.0, vhc_obj.current_orientation]
    vhc_obj.vhc_id = cur_vhc_id
    vhc_obj.set_vehicle_model(vhc_model)
    vhc_obj.color = CAR_COLOR_DICT[2]
    sim_environment.agent_vehicles_list.append(vhc_obj)
    ...
    pedestrian.shirt_color = pedestrian_shirts_color
    pedestrian.pants_color = pedestrian_pants_color
    pedestrian.target_speed = pedestrian_speed
    pedestrian.trajectory = [ped_x_pos, ped_y_pos, ego_x_pos, ped_y_pos, ego_x_pos, ped_y_pos]
    ...
    # Ego Target Path:
    target_pos_list = [[ego_x_pos, -1000.0],
                       [ego_x_pos, 1000.0]]
    ...
    sim_environment.detection_evaluation_config_list.append(DetectionEvaluationConfig(vehicle_id=1,
                                                                                      ...
                                                                                      eval_alg='d_square'))
    sim_environment.detection_evaluation_config_list[-1].add_target_object('pedestrian', 1)
    ...
    sim_environment.periodic_reporting_config_list.append(
        PeriodicReportingConfig(item_type='pedestrian',
                                item_id=PeriodicReportingConfig.ALL_IDS,
                                report_type='position',
                                reporting_period=PeriodicReportingConfig.REPORT_AT_EVERY_STEP))
    sim_environment.periodic_reporting_config_list.append(
...
def run_ca_tests(exp_to_run=None, ca_strength=2):
    ...
    dict_of_parameter_dict = \
        covering_array_utilities.load_parameters_from_covering_array(
            environment_config_dict,
            parameter_name_type_dict=PARAMETER_NAME_TYPE_DICT,
            parameter_conversion_dict=PARAMETER_CONVERSION_DICT,
    ...
    if environment_config_dict['is_save_exp_results_file']:
        results_file_name = environment_config_dict['exp_results_folder'] + environment_config_dict['exp_short_name']\
            + '_ca_' + covering_array_utilities.get_ca_strength_string(ca_strength) + '_way_TEST_RESULTS.csv'
        ...
        try:
            ...
            ca_exp_file_name = environment_config_dict['exp_config_folder'] + environment_config_dict['exp_short_name']\
                + '_ca_' + covering_array_utilities.get_ca_strength_string(ca_strength) + '_way.csv'
    ...
    if environment_config_dict['is_save_exp_results_file']:
        ...
        try:
            ...
            ca_exp_file_name = environment_config_dict['exp_config_folder'] + \
                environment_config_dict['exp_short_name'] + '_ca_2_3_way.csv'
            exp_data_frame = covering_array_utilities.load_experiment_data(ca_exp_file_name, header_line_count=6)
            results_data_frame = exp_data_frame.copy()
        covering_array_utilities.save_experiment_results(results_file_name, results_data_frame)
...
def run_falsification(exp_index, parameter_array, falsif_run_number, staliro_run_count, sim_duration, falsif_test_type):
    exp_index = int(round(exp_index))
    falsif_run_number = int(round(falsif_run_number))
    exp_file_path = os.path.dirname(os.path.realpath(__file__))
    exp_file_name = os.path.splitext(os.path.basename(__file__))[0]
    environment_config_dict = experiment_tools.load_environment_configuration(exp_file_path=exp_file_path,
                                                                              exp_file_name=exp_file_name)
    ...
    ca_exp_file_name = environment_config_dict['exp_config_folder'] + environment_config_dict[
        'exp_short_name'] + '_ca_2_3_way.csv'
    exp_ind = int(round(exp_index))
    data_frame = covering_array_utilities.load_experiment_data(ca_exp_file_name)
    vehicle_model_for_exp = covering_array_utilities.get_experiment_field_value(data_frame, exp_ind, 'agent_1_model')
    parameter_dict['agent_1_model'] = vehicle_model_for_exp
    for (param_ind, param_value) in enumerate(parameter_array):
        parameter_dict[FALSIFICATION_PARAMETER_INDEX_MAP[param_ind]] = param_value
    print('Running Experiment {}'.format(exp_index))
    (trajectory, simulator_instance, sim_environment) = run_single_test(sim_config=sim_config,
...
def plot_data_from_trajectory(trajectory, trajectory_dict):
    ...
    axarr[3].set_title('pedestrian visibility')
    plt.show()


def main():
    run_ca_tests(ca_strength=[2,3])
    exp_file_path = os.path.dirname(os.path.realpath(__file__))
    exp_file_name = os.path.splitext(os.path.basename(__file__))[0]
    environment_config_dict = experiment_tools.load_environment_configuration(exp_file_path=exp_file_path,
                                                                              exp_file_name=exp_file_name)
    ...
    # Following falsifying req 1: (Perception Requirement)
    parameter_array = [29.9711860340428, -0.383588975078234, 2.63652724193123, 0.306405729593619, 0.515393807373457,
                       0.122204173552483, 0.580458003241108, 0.897897552347574, 0.734147504570644, 0.735266636011149,
                       0.496174205459387, 0.725618504356532]
