...
FALSIF_Run_Number = 1;
staliro_run_count = 1;
falsification_n_tests = 500;
currentExpIndex = 0;
Falsif_Test_Type = 'SA';

addpath(genpath('../../../s-taliro'))

load_requirement_R5_scenario_3;
init_cond = get_parameter_ranges_scenario_3();
model = @webots_simulation_ivjournal_scenario_3;
input_range = [];
cp_array = [];
opt = staliro_options();
opt.runs = 1;
...
res_filename = ['results_scenario_3_r5_', datestr(datetime("now"), 'yyyy_mm_dd__HH_MM'), '.mat'];
save(res_filename)
